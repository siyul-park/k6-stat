...
function diff(x: Summary, y: Summary): Summary {
  ...
  const options: Summary['options'] = {
    ...
    summaryTrendStats: arrayUnion(
      x.options.summaryTrendStats,
      y.options.summaryTrendStats,
    ),
  ...
  arrayUnion(Object.keys(x.metrics), Object.keys(y.metrics)).forEach(
    (metricKey) => {
      const xValues = x.metrics[metricKey]?.values ?? {};
      const yValues = y.metrics[metricKey]?.values ?? {};

      const type =
        y.metrics[metricKey]?.type ?? x.metrics[metricKey]?.type ?? '';
      const contains =
        y.metrics[metricKey]?.contains ?? x.metrics[metricKey]?.contains ?? '';

      const values: Record<string, number> = {};
      arrayUnion(Object.keys(xValues), Object.keys(yValues)).forEach(
        (valueKey) => {
          values[valueKey] =
            (yValues[valueKey] ?? 0) - (xValues[valueKey] ?? 0);
        },
      );
